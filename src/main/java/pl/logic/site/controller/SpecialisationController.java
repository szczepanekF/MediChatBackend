...
public class SpecialisationController {
     ...
     * @return HTTP response
     */
    @GetMapping(value = "/specialisations/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Get specialisation from the database", description = "Get specialisation from the database")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved"),
            @ApiResponse(responseCode = "404", description = "Not found")
    })
    public ResponseEntity<Response> getSpecialisation(@Parameter(description = "id of specialisation to be searched") @PathVariable int id) {
        Specialisation specialisation = new Specialisation();
        try {
            specialisation = (Specialisation) objectFacade.getObject(new SpecialisationDAO(new Specialisation()), id);
     ...
     */
    @ResponseBody
    @PutMapping(value = "/specialisations/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public ResponseEntity<Response> updateSpecialisation(@Parameter(description = "id of specialisation to be searched") @PathVariable int id, @RequestBody SpecialisationDAO specialisationDAO) {
        Specialisation specialisation = new Specialisation();
        try {
            specialisation = (Specialisation) objectFacade.updateObject(specialisationDAO, id);
     ...
     */
    @ResponseBody
    @DeleteMapping(value = "/specialisations/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public ResponseEntity<Response> deleteSpecialisation(@Parameter(description = "id of specialisation to be searched") @PathVariable int id) {
        Specialisation specialisation = new Specialisation();
        try {
            objectFacade.deleteObject(new SpecialisationDAO(new Specialisation()), id);
            // Update specialisation logic here
